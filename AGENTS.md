...
To run the custom suite of unit test for Mailme use the following sequence of commands that will install dependencies
...
- Increment (look at `CHANGELOG.md` for semver changes) the `version` value in `setup.py`, and then the `def _version(self):` return value in `src/mailme/main.py`.
...
Mailme is licensed under the [Apache License, Version 2.0](http://www.apache.org/licenses/).
